...
router.post('/', (req, res) => {
  ...
  // Verify referenced records exist before inserting (FK constraints on Vercel ephemeral DB)
  if (client_id) {
    const exists = db.prepare('SELECT id FROM clients WHERE id = ?').get(client_id);
    if (!exists) return res.status(404).json({ error: 'Client not found — the database may have reseeded. Please reload the page.' });
  }
  if (property_id) {
    const exists = db.prepare('SELECT id FROM properties WHERE id = ?').get(property_id);
    if (!exists) return res.status(404).json({ error: 'Property not found — the database may have reseeded. Please reload the page.' });
  }
